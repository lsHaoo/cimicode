...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("cimicode://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("cimicode://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("cimicode://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("cimicode://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("cimicode://open-project")).toBeUndefined()
    expect(parseDeepLink("cimicode://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "cimicode://open-project?directory=/a",
      "cimicode://other?directory=/b",
      "cimicode://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("cimicode://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("cimicode://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("cimicode://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("cimicode://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "cimicode://new-session?directory=/a",
      "cimicode://open-project?directory=/b",
      "cimicode://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __CIMICODE__: {
        deepLinks: ["cimicode://open-project?directory=/a"],
      },
    } as unknown as Window & { __CIMICODE__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["cimicode://open-project?directory=/a"])
