...
This skill uses npm packages from your organization's internal npm registry. **Before installing or using, configure npm to use the internal registry:**

```bash
npm config set registry http://172.16.13.24/repository/npm/
```

All npm/npx commands used by this skill will automatically use this configured registry.

## Dependencies

- Node.js + npm/npx
- Optional: draw.io Desktop for PNG/PDF export
- Optional: `@next-ai-drawio/mcp-server` for live inline editing
